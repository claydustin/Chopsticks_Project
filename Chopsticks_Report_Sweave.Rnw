...
Figure \ref{fig:categorical} displays boxplots for four predictor variables. Tip variation appears to vary across all variables; race, age, gender, and payment type. Under age the most variation is between young (Y) and old (O) and young and middle age customers. There is much more variation in cash payments (C) compared to credit card (R) payments. Tip varies the most according to race, however. 
While collecting tips I noted that interaction terms could further explain some of the association among predictor variables. For instance, the effect of distance travelled on tip amount seemed to vary with race. White and latino customersfarther away from Chopsticks typically accounted for the additional travel in their tip whereas black customers tipped the same regardless of distance travelled. 
